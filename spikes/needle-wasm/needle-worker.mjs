...
const cactName = readdirSync(workerData.engineDir).find((f) => f.endsWith('.cact'));
const cact = readFileSync(join(workerData.engineDir, cactName));
...
parentPort.postMessage({ ready: loadRc >= 0, loadRc, ms: performance.now() - loadStart, cactName, heapMb: heapMb() });
...
parentPort.on('message', ({ id, op, ...args }) => {
    ...
    try {
        if (op === 'init') {
            ...
            parentPort.postMessage({ id, ok: true, prefixTokens: rc, ms: performance.now() - t0, heapMb: heapMb() });
        } else if (op === 'complete') {
            ...
            parentPort.postMessage({ id, ok: true, envelope: JSON.parse(raw), ms: performance.now() - t0, heapMb: heapMb() });
        } else {
